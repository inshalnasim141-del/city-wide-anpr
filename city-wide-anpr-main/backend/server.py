...
async def get_current_user(request: Request) -> dict:
    if db is None or len(JWT_SECRET) < 32:
...
class BlacklistCreate(BaseModel):
    ...
    notes: Optional[str] = Field(default="", max_length=2_000)
...
class AlertActionRequest(BaseModel):
    ...
    note: Optional[str] = Field(default="", max_length=2_000)


def normalize_plate(plate: str) -> str:
    normalized = plate.upper().strip()
    if not re.fullmatch(r"[A-Z0-9-]{4,16}", normalized):
        raise HTTPException(status_code=422, detail="Plate must contain 4-16 letters, numbers, or hyphens")
    return normalized
...
async def login(payload: LoginRequest, response: Response):
    if db is None or len(JWT_SECRET) < 32:
        raise HTTPException(
            status_code=503,
            detail="Authentication is not configured. Add MONGO_URL, DB_NAME, and JWT_SECRET in Vercel.",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail="Authentication service is unavailable. Verify the MONGO_URL credentials in Vercel.",
        )
...
async def list_detections(
    ...
    q = {}
    if plate:
        q["plate"] = {"$regex": re.escape(normalize_plate(plate)), "$options": "i"}
    if camera_id:
...
async def delete_blacklist(plate: str, user: dict = Depends(get_current_user)):
    if user["role"] not in ("admin", "investigator"):
        raise HTTPException(status_code=403, detail="Only investigators/admins may modify blacklist")
    result = await db.blacklist.delete_one({"plate": normalize_plate(plate)})
...
async def on_startup():
    if db is None:
        logger.warning("Database is not configured; skipping ANPR seed.")
        return
    try:
        ...
        # A database outage or invalid deployment credential must not make the
        # entire serverless function unavailable (including its health check).
        logger.exception("Database startup failed; API data routes remain unavailable until MongoDB is configured.")
...
# Vercel runs this as one FastAPI application. Serve the React production build
# after registering API routes so /api/* always takes precedence.
...
async def frontend(path: str):
    ...
    if FRONTEND_BUILD_DIR in requested_file.parents and requested_file.is_file():
