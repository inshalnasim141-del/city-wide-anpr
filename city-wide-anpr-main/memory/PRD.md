...
## User Personas / Roles
- **Admin** — full access, user and blacklist management. Provision the account through the deployment environment; never commit credentials.
- **Investigator** — trajectory tracking, alert triage, and blacklist management.
- **Operator** — live monitoring and alert triage only (no blacklist writes).
